...
export async function sendConfirmationEmail(data: BookingEmailData) {
  ...
  const extrasText =
    data.extras.length > 0
      ? data.extras
          .map((e) => `- ${e.name}: ${formatZAR(e.price)}`)
          .join("<br/>")
      : "None";

  const serviceLabel =
    data.serviceType === "from_airport"
    ...
    body: JSON.stringify({
      ...
      html: `
        ...
        <table style="border-collapse:collapse;width:100%;">
          ...
          <tr><td style="padding:8px;font-weight:bold;">Date</td><td style="padding:8px;">${
            data.pickupDate
          }</td></tr>
          <tr><td style="padding:8px;font-weight:bold;">Time</td><td style="padding:8px;">${
            data.pickupTime
          }</td></tr>
          ...
          ${
            data.specialRequests
              ? `<tr><td style="padding:8px;font-weight:bold;">Special Requests</td><td style="padding:8px;">${data.specialRequests}</td></tr>`
              : ""
          }
